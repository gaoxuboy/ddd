        ...
        for option in self.model_config.list_filter:
            ...
            if option.is_func:
                ...
                if isinstance(_field, ForeignKey):
                    data_list = FilterRow(option, self, _field.rel.model.objects.all(), self.model_config.request.GET)
                elif isinstance(_field, ManyToManyField):
                    data_list = FilterRow(option, self, _field.rel.model.objects.all(), self.model_config.request.GET)
                else:
                    data_list = FilterRow(option, self, _field.model.objects.all(), self.model_config.request.GET)
            yield data_list
